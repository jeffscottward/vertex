...
export function UI() {
  ...
  return (
    ...
    >
      ...
      {/* Overdrive Bar - only when playing */}
      {isPlaying && (
        <div
          ...
          <div data-component-id="ui-lock-indicators" style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '15px' }}>
            {Array.from({ length: 8 }, (_, i) => (
              <div key={i} style={{
                ...
                height: '12px',
                borderRadius: '50%',
                border: `2px solid ${i < lockedTargets.length ? '#ff00ff' : '#00ffff'}`,
                background: i < lockedTargets.length ? '#ff00ff' : 'transparent',
                boxShadow: i < lockedTargets.length ? '0 0 8px #ff00ff' : 'none',
                transition: 'all 0.1s ease',
              }} />
            ))}
          </div>
      ...
      {isGameOver && (
        ...
        >
          ...
          <div style={{ fontSize: '24px', marginTop: '20px', color: '#00ffff' }}>
